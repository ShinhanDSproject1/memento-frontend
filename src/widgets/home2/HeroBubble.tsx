...
export function HeroBubble({ text, highlight, rotateTexts, intervalMs = 30_000, role }: Props) {
  ...
  if (isRecommendChat) {
    return (
      <motion.div
        className="relative h-[220px] w-full overflow-hidden rounded-2xl border border-blue-200 bg-white/95 px-4 py-5 shadow-md backdrop-blur sm:h-[240px] md:h-[260px]" // ✅ 고정에 가깝게 더 크게
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
        {/* 스크롤 가능한 텍스트 컨테이너(상단 정렬) */}
        <div
          className="mx-auto h-full max-w-[720px] cursor-text touch-pan-y overflow-auto pr-1 text-center text-[15px] leading-6 text-[#23272E] select-text [scrollbar-gutter:stable]"
